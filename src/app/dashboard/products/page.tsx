            ...
            <tbody className="divide-y divide-slate-100">
              {products.map((p: any) => (
                <tr key={p.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3"><span className={`rounded-full px-2 py-0.5 text-[10px] font-bold ${p.status === "ACTIVE" ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-600"}`}>{p.status === "ACTIVE" ? "AKTIF" : "NONAKTIF"}</span></td>
                  {canManage && (
                    <td className="px-4 py-3 text-right">
                      <button className="mr-2 text-slate-400 hover:text-teal-600" onClick={() => openEdit(p)}><Pencil size={15} /></button>
                      <button className="text-slate-400 hover:text-red-600" onClick={() => { if (confirm("Hapus produk ini?")) deleteMut.mutate(p.id); }}><Trash2 size={15} /></button>
                    </td>
                  )}
                </tr>
              ))}
