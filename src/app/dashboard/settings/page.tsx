...
import { Save, Loader2, Store, CreditCard, Receipt } from "lucide-react";
...
export default function SettingsPage() {
  ...
  const [form, setForm] = useState({ name: "", address: "", phone: "", email: "", businessHours: "", currency: "IDR", taxRate: 0 });

  useEffect(() => {
    if (settings) setForm({ name: settings.name || "", address: settings.address || "", phone: settings.phone || "", email: settings.email || "", businessHours: settings.businessHours || "", currency: settings.currency || "IDR", taxRate: settings.taxRate || 0 });
  }, [settings]);
