        ...
        <div className="overflow-x-auto rounded-xl border border-slate-200 bg-white shadow-sm">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              ...
              {transactions.length === 0 && <tr><td colSpan={7} className="px-4 py-12 text-center text-slate-400">Transaksi tidak ditemukan</td></tr>}
            </tbody>
          </table>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/50 p-4" onClick={() => setDetail(null)}>
          <div className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-xl bg-white p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            {canManage && detail.status === "COMPLETED" && (
              <button className="mt-3 flex h-10 w-full items-center justify-center gap-2 rounded-lg border border-rose-300 text-sm font-semibold text-rose-700 hover:bg-rose-50" onClick={() => { if (confirm("Refund transaksi ini?")) refundMut.mutate(detail.id); }}>
